            ...
            assignedDevices.map(device => (
              <div key={device.device_id} className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${device.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'
                    }`}>
                    {device.status}
                  </span>
                </div>
                <button
                  onClick={() => navigate(`/devices/${device.device_id}`, { state: { from: `/faculty/${id}`, label: facultyMember.faculty_name } })}
